    ...
    if st.session_state.lanche_atual["n"] or st.session_state.lanche_atual["extras"]:
        with st.container():
            ...
            if st.session_state.lanche_atual["n"]:
                st.write(f"🍔 **{st.session_state.lanche_atual['n']}**")
                st.caption(f"({st.session_state.lanche_atual['ing']}) - R$ {st.session_state.lanche_atual['p']:.2f}")
...
with tabs[4]:
    ...
    for idx, l in enumerate(st.session_state.lanches_fechados):
        ...
        total_ped += sub
        st.markdown(f"""<div style="background:#e3f2fd; padding:10px; border-radius:8px; margin-bottom:5px;">
        ✅ <b>LANCHE {idx+1}: {l['n']}</b><br>
        <small><i>({l['ing']})</i></small><br>
    ...
    if st.session_state.lanche_atual["n"]:
        ...
        st.markdown(f"""<div style="background:#fff3e0; padding:10px; border-radius:8px;">
        ⏳ <b>MONTANDO AGORA (Lanche {st.session_state.id_atual}): {st.session_state.lanche_atual["n"]}</b><br>
        <small><i>({st.session_state.lanche_atual['ing']})</i></small><br>
        {f"".join([f"• {e['n']} (R$ {e['p']:.2f})<br>" for e in st.session_state.lanche_atual['extras']])}
        <b>Subtotal Parcial: R$ {sub_atual:.2f}</b></div>""", unsafe_allow_html=True)
    ...
    if st.button("🟢 ENVIAR PEDIDO COMPLETO", type="primary"):
        total_final = total_ped + (st.session_state.lanche_atual['p'] + sum(e['p'] for e in st.session_state.lanche_atual['extras']) if st.session_state.lanche_atual['n'] else 0)
        if nome_u and end_u and total_final > 0:
            if st.session_state.lanche_atual["n"]:
                l_f = st.session_state.lanche_atual
                resumo_whats += f"\n*LANCHE {st.session_state.id_atual}: {l_f['n']}*\n({l_f['ing']})\n" + "".join([f"- {e['n']}\n" for e in l_f['extras']])
...
rodape_val = total_ped + (st.session_state.lanche_atual['p'] + sum(e['p'] for e in st.session_state.lanche_atual['extras']) if st.session_state.lanche_atual['n'] else 0)
if rodape_val > 0:
