...
# =========================================================
# 🛠️ CARDÁPIO COMPLETO (RECUPERADO DA FOTO ORIGINAL)
# =========================================================
...
with col_menu:
        ...
        with tabs[2]:
            for n, p in BEBIDAS.items():
                if st.button(f"🥤 {n} - R$ {p:.2f}", key=f"beb_{n}"):
                    st.session_state.lanche_atual["extras"].append({"n": f"Bebida {n}", "p": p})
                    st.rerun()
    ...
    with tabs[4]:
        ...
        if len(pedidos) > 0 and nome and tel and end:
            ...
            st.link_button("🟢 ENVIAR WHATSAPP", f"https://wa.me/{WHATSAPP_ALAN}?text={msg.replace(' ', '%20').replace('\n', '%0A')}")
            if st.button("🔄 NOVO PEDIDO (LIMPAR SITE)"): resetar_tudo()
        else:
            st.warning("Preencha os dados para liberar o envio.")

# =========================================================
# 📋 PAINEL VISUAL (FOTOCÓPIA DA SUA NECESSIDADE)
# =========================================================
with col_visual:
    ...
    if st.session_state.id_atual <= 6:
        st.markdown(f"### ⏳ MONTANDO (Lanche {st.session_state.id_atual})")
        if st.session_state.lanche_atual["n"]:
            l_at = st.session_state.lanche_atual
            st.info(f"**{l_at['n']}** (R$ {l_at['p']:.2f})")
            sub_p = l_at['p']
            for i, e in enumerate(l_at["extras"]):
                ...
                if c2.button("🗑️", key=f"del_{i}"):
                    ...
                    st.rerun()
            st.write(f"**Subtotal Atual: R$ {sub_p:.2f}**")
            if st.button(f"🔒 FECHAR LANCHE {st.session_state.id_atual}", use_container_width=True):
